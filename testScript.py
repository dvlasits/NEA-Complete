...
def Finding(c1,c2):
    smallx  = min([c[0] for c in [c1,c2]])
    bigx =  max([c[0] for c in [c1,c2]])
    smally  = min([c[1] for c in [c1,c2]])
    bigy =  max([c[1] for c in [c1,c2]])
    return smallx,bigx,smally,bigy

def Turn(c1,c2,lights, type):
    def moreWork():
        pass
    moreWork()
    global totalTurn
    totalTurn += 1
    smallx,bigx,smally,bigy = Finding(c1,c2)
...
def Toggle(c1,c2,lights):
    global totalToggle
    totalToggle += 1
    smallx,bigx,smally,bigy = Finding(c1,c2)# kjjjjjjjjj
    for x in range(smallx,bigx+1):
        for y in range(smally,bigy+1):
            lights[y][x] = 1-lights[y][x]
#JFFLJJKJJKFKJFKJFFKs
def main():
    for row in data.splitlines():
        items = row.split()
        dictionary = {"on":1,"off":0}
        if items[1] in dictionary:
            c1 = [int(i) for i in items[2].split(',')]
            c2 = [int(i) for i in items[4].split(',')]
            Turn(c1,c2,lights,dictionary[items[1]])
        else:
            c1 = [int(i) for i in items[1].split(',')] #UGFKHDFJDFJKFDJKDFJKKJDFKJFKJFKJF
            c2 = [int(i) for i in items[3].split(',')]
            Toggle(c1,c2,lights)
...
print(totalToggle)
#this is a comments
print("#this is not an acutal comment")
