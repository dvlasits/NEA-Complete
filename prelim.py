...
class Outlet:
  ...
  def NewDay(self):
    self._VisitsToday = 0
    self.DeliveryCosts = 0

  def CalculateDailyProfitLoss(self, AvgCostPerMeal, AvgPricePerMeal):
    return (AvgPricePerMeal - AvgCostPerMeal) * self._VisitsToday - self._DailyCosts - self.DeliveryCosts

  def GetDetails(self):
    Details = "Coordinates: (" + str(self._XCoord) + ", " + str(self._YCoord) + ")     Capacity: " + str(self._Capacity) + "      Maximum Capacity: "
    Details += str(self._MaxCapacity) + "      Daily Costs: " + str(self._DailyCosts) + "      Visits today: " + str(self._VisitsToday) + f"DeliveryCosts + {self.DeliveryCosts}"
    return Details

  def AddDelivery(self,distance,fuelCost):
      self.DeliveryCosts += distance*fuelCost
